    ...
    <div class="mui-card">
      ...
      <div class="mui-card-content">
        <div class="mui-card-content-inner">
          ...
          <p>
            <mt-button type="primaary" size="small">立即购买</mt-button>
...
export default {
  data () {
    return {
      id: this.$route.params.id, //将路由参数对象中的id挂载到data, 方便后期掉用
      lunbotu: [], //轮播图的数据
      goodsinfo: {}, //获取到的商品的信息
      ballFlag: false, //控制小球的显示和隐藏的标识符
      buyCount: 1 // 保存用户选中的商品数量, 默认,用户购买一个
    };
  },
  created() {
    this.getLunbotu();
  ...
  methods: {
    ...
    getGoodsInfo() {
      //获取商品的信息
      this.$http.get("api/goods/getinf0/" + this.id).then(result => {
        if(result.body.status === 0) {
    ...
    addToShopCar() {
      ...
      var goodsInfo = {
        id: this.id,
        count: this.buyCount,
        price: this.goodsinfo.sell_ptice,
        selected: true
      };
      // 调用 store 中的 mutations 来将商品加入购物车
      this.$route.commit("addToCar", goodsInfo);
    },
    boforeEnter(el) {
    ...
    enter(el, done) {
      ...
      el.style.transition = "all 0.5s cubic-bezier (.4, -0.3, .68)";
      done();
    },
    afterEnter(el) {
      this.ballFlag = !this.ballFlag;
